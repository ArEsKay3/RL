...
# 97.7%. Without a cache the coordinator ignores affinity anyway, so resolve to
# load_balanced there rather than having frontends hash prompts nobody reads.
...
class MegatronGenerationMixin:
    ...
    def _initialize_inference_engine(self, mcore_generation_config: dict) -> None:
        ...
        inference_config = InferenceConfig(
            ...
            # Ceiling on the prefill/mixed graph range. mcore defaults this to 512, so
            # any prefill step longer than that runs eager -- on a 60-turn SWE workload
    ...
    def _setup_openai_api_server(self) -> str:
        ...
        # then converge on one port, and because each frontend replica binds with
        # SO_REUSEPORT the duplicate bind succeeds instead of failing. The result is
        # several ranks advertising one address, which collapses into a single
        # client-side connection pool.
        dp_rank = get_pg_rank(self.dynamic_inference_engine.pg_collection.dp)
        free_port = _get_free_port_local(rng=random.Random(dp_rank))
    ...
    def _run_async_coordinator_start(self):
        ...
        # One frontend per model-parallel group rather than one for the whole job.
        # Frontend work (chat template, detokenize, parsers, JSON) is CPU-bound, so
        # hosting it on a single rank caps request throughput at that rank's cores;
        # spreading it also gives the client one address per group instead of one
        # for everything. Hosting on *every* rank was measured to add nothing over
        # this at r2, and multiplies the process count by the TP degree.
        if (
